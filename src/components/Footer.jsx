...
const Footer = () => {
  return (
    <footer className='bg-footer bg-cover bg-no-repeat pt-16 relative'>
        {/* overlay */}
        <div className='h-full w-full bg-black/[0.90] absolute top-0 z-10' ></div>
            ...
            <div className='flex flex-col items-center justify-center gap-14'>
                ...
                <nav className='flex flex-col xl:flex-row gap-8 xl:gap-12 justify-center items-center'>
                    {links.map((link,index)=>{
                        return <Link href={link.href} key={index} className='uppercase text-white tracking-widest hover:text-accent transition-all'>
                            {link.name}
                        </Link>
                    })}
                </nav>
